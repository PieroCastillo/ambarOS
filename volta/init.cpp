...
ambarOSfn size_t strlen(const char* str) 
{
	size_t len = 0;
	while (str[len])
		len++;
...
ambarOSfn void terminalPutEntryAt(char c, uint8_t color, size_t x, size_t y) 
{
	const size_t index = y * VGA_WIDTH + x;
	terminal_buffer[index] = vgaEntry(c, color);
}

ambarOSfn void terminalPutChar(char c) 
{
...
ambarOSfn void terminalWrite(const char* data, size_t size) 
{
	for (size_t i = 0; i < size; i++)
		terminalPutChar(data[i]);
}

ambarOSfn void terminalWriteString(const char* data) 
{
...
{
    terminalInitialize();
	terminalWriteString("amberOS alpha v0.0.1\n");
